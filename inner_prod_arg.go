...
func GenerateNewParams(EC *CryptoParams, G, H []ECPoint, x *big.Int, L, R, P ECPoint) ([]ECPoint, []ECPoint, ECPoint) {
	nprime := len(G) / 2
...
*/
func InnerProductProveSub(EC *CryptoParams, proof InnerProdArg, G, H []ECPoint, a []*big.Int, b []*big.Int, u ECPoint, P ECPoint) InnerProdArg {
	...
	//fmt.Println(nprime)
	//fmt.Println(len(H))
	cl := InnerProduct(a[:nprime], b[nprime:], EC.N) // either this line
	cr := InnerProduct(a[nprime:], b[:nprime], EC.N) // or this line
	L := EC.TwoVectorPCommitWithGens(G[nprime:], H[:nprime], a[:nprime], b[nprime:]).Add(u.Mult(cl))
	R := EC.TwoVectorPCommitWithGens(G[:nprime], H[nprime:], a[nprime:], b[:nprime]).Add(u.Mult(cr))
	...
	Gprime, Hprime, Pprime := GenerateNewParams(EC, G, H, x, L, R, P)
	//fmt.Printf("Prover - Intermediate Pprime value: %s \n", Pprime)
	xinv := new(big.Int).ModInverse(x, EC.N)

	// or these two lines
	aprime := VectorAdd(
		ScalarVectorMul(a[:nprime], x, EC.N),
		ScalarVectorMul(a[nprime:], xinv, EC.N), EC.N)
	bprime := VectorAdd(
		ScalarVectorMul(b[:nprime], xinv, EC.N),
		ScalarVectorMul(b[nprime:], x, EC.N), EC.N)

	return InnerProductProveSub(EC, proof, Gprime, Hprime, aprime, bprime, u, Pprime)
}

func InnerProductProve(EC *CryptoParams, a []*big.Int, b []*big.Int, c *big.Int, P, U ECPoint, G, H []ECPoint) InnerProdArg {
	...
	return InnerProductProveSub(EC, runningProof, G, H, a, b, ux, Pprime)
}
...
*/
func InnerProductVerify(EC *CryptoParams, c *big.Int, P, U ECPoint, G, H []ECPoint, ipp InnerProdArg) bool {
	...
	for curIt >= 0 {
		...
		Gprime, Hprime, Pprime = GenerateNewParams(EC, Gprime, Hprime, chal2, Lval, Rval, Pprime)
		curIt -= 1
	}
...
func InnerProductVerifyFast(EC *CryptoParams, c *big.Int, P, U ECPoint, G, H []ECPoint, ipp InnerProdArg) bool {
	//fmt.Println("Verifying Inner Product Argument")
	//fmt.Printf("Commitment Value: %s \n", P)
	s1 := sha256.Sum256([]byte(P.X.String() + P.Y.String()))
	chal1 := new(big.Int).SetBytes(s1[:])
	...
	*/
	...
	ccalc := new(big.Int).Mod(new(big.Int).Mul(ipp.A, ipp.B), EC.N)
	lhs := EC.TwoVectorPCommitWithGens(G, H, ScalarVectorMul(sScalars, ipp.A, EC.N), ScalarVectorMul(invsScalars, ipp.B, EC.N)).Add(ux.Mult(ccalc))
